...
<div class="forumHeader">Forum</div>
		...
		<div class="forumRow fancy">
			<div class="forumLeftcolumn">
				<div class="forumCard" style="padding-left: 1em;">
					<h2>${item.title}</h2>
					<h5>
						<b>Created on: <fmt:formatDate value="${item.post_created}" /></b> By: ${item.name}
					</h5>
					${item.text}<br> <a href="/user/replytopost?reference=${item.reference}&parent=0&level=0">Reply</a>
					<c:choose>
						<c:when test="${whoAmI == item.author}">
				...
				<c:forEach var="row" items="${result.rows}">
					...
					<div style="padding-left: ${indent * 3}em">
						<b>Created on: <fmt:formatDate value="${row.post_created}" /></b> By: ${row.name}<br> ${row.text}<br> <a
							href="/user/replytopost?reference=${row.reference}&parent=${row.id}&level=${row.level}">Reply</a>
						<c:choose>
							<c:when test="${whoAmI == row.author}">
